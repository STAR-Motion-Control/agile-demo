...
class CommandsCfg:
    ...
    base_velocity = mdp.UniformVelocityBaseHeightCommandCfg(
        ...
        ranges=mdp.UniformVelocityBaseHeightCommandCfg.Ranges(
            ...
            base_height=(0.20, unitree_g1.DEFAULT_PELVIS_HEIGHT),  # deep-squat: floor 0.40 -> 0.20
        ),
        min_walk_height=0.5,
...
class RewardsCfg:
    ...
    track_base_height_exp_smooth = RewTerm(
        ...
        weight=1.0,
        params={"command_name": "base_velocity", "std": 0.35},  # deep-squat: keep gradient far from target (0.2 -> 0.35)
    )
    ...
    ankle_pos_limits = RewTerm(
        func=mdp.joint_pos_limits,
        weight=-2.0,  # deep-squat: ankles need deep dorsiflexion / heel-lift when low (was -10.0)
    ...
    flat_orientation = RewTerm(
        ...
        weight=-2.0,  # deep-squat: forward torso lean is required when low (was -10.0)
...
class TerminationsCfg:
    ...
    base_orientation = DoneTerm(
        ...
        params={
            ...
            "limit_angle": math.radians(55.0),  # deep-squat: allow forward torso lean (40 -> 55 deg)
        },
    )
        ...
        params={
            "sensor_cfg": SceneEntityCfg(
                ...
                body_names=[
                    ...
                    "pelvis",
                    ".*_hip_.*_link",
                    # ".*_knee_link",  # deep-squat: knees may graze ground when low; rely on min_height gate
                ],
            ),
            "asset_cfg": SceneEntityCfg("robot", body_names=["torso_link"]),
            "threshold": 50.0,
            "min_height": 0.25,  # deep-squat: only flag collapse below torso 0.25m (was 0.5)
        },
    )
    ...
    knee_distance = DoneTerm(
        ...
        params={
            ...
            "min_distance_threshold": 0.08,  # deep-squat: knees come closer when low (0.2 -> 0.08)
        },
    )
...
def _apply_g1_23dof(env_cfg):
    """Swap to the 23-DoF G1 and fix the 2 refs to 29-DoF-only joints/bodies.

    23-DoF (Unitree base G1) lacks waist roll/pitch and wrist pitch/yaw; the legs
    (the only actuated joints in this lower-body task) are identical to 29-DoF.
    """
    env_cfg.scene.robot = unitree_g1.G1_23DOF.replace(prim_path="{ENV_REGEX_NS}/Robot")
    # random_upper_body_pos: drop waist roll/pitch limits (joints absent on 23-DoF)
    _lim = env_cfg.actions.random_upper_body_pos.joint_pos_limits
    for _k in ("waist_roll_joint", "waist_pitch_joint"):
        if _k in _lim:
            del _lim[_k]
    # external-force extremities: 23-DoF arm tip is wrist_roll_rubber_hand, not wrist_yaw_link
    if hasattr(env_cfg.events, "apply_external_force_torque_extremities"):
        env_cfg.events.apply_external_force_torque_extremities.params["asset_cfg"] = SceneEntityCfg(
            "robot", body_names=[".*wrist_roll.*"]
        )


@configclass
class G1Lower23dofVelocityHeightEnvCfg(G1LowerVelocityHeightEnvCfg):
    """23-DoF (Unitree base G1) variant of the deep-squat velocity-height teacher env."""

    def __post_init__(self):
        super().__post_init__()
        _apply_g1_23dof(self)


@configclass
class G1VelocityHeight23dofRecurrentStudentEnvCfg(G1VelocityHeightRecurrentStudentEnvCfg):
    """23-DoF distillation (recurrent student) env."""
