...
# ---------------------------------------------------------------------------
# G1 23-DoF (Unitree base G1): 12 legs + waist_yaw + 10 arms (shoulder p/r/y,
# elbow, wrist_roll). USD converted from Unitree official g1_23dof.urdf
# (unitree_rl_gym). Derived from G1_29DOF by swapping the USD and pruning the
# 6 joints absent on 23-DoF (waist roll/pitch, wrist pitch/yaw) from the
# actuator dicts. Legs/feet are identical to 29-DoF, so G1_ACTION_SCALE_LOWER
# (leg+feet only) applies unchanged.
# ---------------------------------------------------------------------------
G1_23DOF_USD_PATH = "/hdd0/lizhe/g1_deepsquat/g1_23dof_usd/g1_23dof.usd"
G1_23DOF = _copy.deepcopy(G1_29DOF)
G1_23DOF.spawn.usd_path = G1_23DOF_USD_PATH
G1_23DOF.spawn.variants = None  # converted USD has no hand variants
for _k in ("waist_roll_joint", "waist_pitch_joint"):
    for _d in (
        G1_23DOF.actuators["waist"].effort_limit_sim,
        G1_23DOF.actuators["waist"].velocity_limit_sim,
        G1_23DOF.actuators["waist"].stiffness,
        G1_23DOF.actuators["waist"].damping,
    ):
        if isinstance(_d, dict):
            _d.pop(_k, None)
for _k in (".*_wrist_pitch_joint", ".*_wrist_yaw_joint"):
    for _d in (
        G1_23DOF.actuators["arms"].effort_limit_sim,
        G1_23DOF.actuators["arms"].velocity_limit_sim,
    ):
        if isinstance(_d, dict):
            _d.pop(_k, None)
