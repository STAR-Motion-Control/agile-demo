#!/usr/bin/env python3
"""
GR00T-WholeBodyControl decoupled-WBC G1 benchmark harness — unified spec v1.

Runs NVIDIA's GR00T-WholeBodyControl decoupled-WBC controller (a DUAL onnx
policy: Balance when the locomotion command is ~zero, Walk otherwise; 516-d
obs history -> 15 lower-body actions, 50 Hz) in MuJoCo (200 Hz physics, the
standard 29-dof scene_29dof scene), fully headless. The obs/action/PD pipeline
is transplanted verbatim from the validated smoke_dwbc.py (run_mujoco_gear_wbc
equivalent); only the GUI viewer is swapped for mujoco.Renderer (EGL offscreen)
and the test battery / scoring / box welds / arena come from the shared
unified-spec scaffold (matching bench_homie / bench_agile / bench_falcon).

Lower body (legs 12 + waist 3 = 15) is policy-controlled; the 14 arms are PD-
held at arm_target (zeros by default, or psi0-replayed arm angles). The waist
stays policy-controlled even in psi0 tests (only the 14 arm columns of the npz
are replayed). The scene is scene_29dof (nq=36, nu=29) — the same file
bench_agile / bench_falcon use, so the box welds + arena furniture inject
against identical body names.

Runtime environment (4090 server, headless):
    ssh 4090
    cd /sda/lizhe/g1bench
    # env: /sda/lizhe/miniforge3/envs/homie/bin/python
    #   (mujoco 3.9 + onnxruntime + numpy; NO torch — numpy only)
    # weights: GR00T-WholeBodyControl-{Balance,Walk}.onnx (--model-dir)
    # scene:   unitree_mujoco/unitree_robots/g1/scene_29dof.xml (--mjcf)

Example commands (from /sda/lizhe/g1bench):
    MUJOCO_GL=egl python bench_dwbc.py --test walk_speed   --trials 50 \
        --out-dir results/dwbc/walk_speed   --video policy
    MUJOCO_GL=egl python bench_dwbc.py --test squat_box    --trials 50 \
        --out-dir results/dwbc/squat_box    --video policy
    MUJOCO_GL=egl python bench_dwbc.py --test circle_pillar --trials 50 \
        --out-dir results/dwbc/circle_pillar --video policy
    MUJOCO_GL=egl python bench_dwbc.py --test speed_sweep  --trials 5 \
        --out-dir results/dwbc/speed_sweep  --video policy
    # quick smoke test:
    MUJOCO_GL=egl python bench_dwbc.py --test walk_speed --trials 2 --video none

Outputs in --out-dir:
    results.jsonl   one line per trial
    summary.json    aggregates (success rate, mean+/-std per metric)
    videos/{label}_{test}_t{NN}_{ok|fail}.mp4   per video policy
...
the 14 arms replay a Psi0 trajectory frame-by-frame (--upper-replay npz,
contract v1, agile29 K=17 = arms14 + waist3; only the 14 arm columns are
mapped by joint name onto qpos[22:36], the 3 waist columns are DROPPED since
the waist is policy-controlled) through the arm PD path. walk/circle psi0:
real_ep053 looped, replay height_cmd ignored (legs
...
never with the robot (bit 1), per the unified psi0 spec.
    ...
    MUJOCO_GL=egl python bench_dwbc.py --test squat_box_psi0 --trials 50 \
        --upper-replay upper_replay_agile29_sim_ep035.npz \
        --out-dir results/dwbc/squat_box_psi0 --video policy
...
v3 tests (spec v3; none of them need a psi0 npz):
  ...
  T6 pipeline_abc  box welded from t=0 (standing hug) -> NAV to
    ...
    MUJOCO_GL=egl python bench_dwbc.py --test squat_sweep --trials 5 \
        --out-dir results/dwbc/squat_sweep --video policy
    MUJOCO_GL=egl python bench_dwbc.py --test goto_ab --trials 25 \
        --out-dir results/dwbc/goto_ab --video policy
    MUJOCO_GL=egl python bench_dwbc.py --test pipeline_abc --trials 15 \
        --out-dir results/dwbc/pipeline_abc --video policy
...
v4 tests (spec v4):
  ...
  T8 squat_place_psi0  Psi0-coordinated reach-and-place: carry the 2 kg
      ...
      --upper-replay upper_replay_agile29_real_ep053*.npz.

    MUJOCO_GL=egl python bench_dwbc.py --test squat_limit --trials 10 \
        --out-dir results/dwbc/squat_limit --video all
    MUJOCO_GL=egl python bench_dwbc.py --test squat_place_psi0 --trials 25 \
        --upper-replay upper_replay_agile29_real_ep053.npz \
        --out-dir results/dwbc/squat_place_psi0 --video policy
...
v5 tests (spec v5):
  ...
  T10 vln_follow  irregular small-command stream following (simulated VLN
    ...
    MUJOCO_GL=egl python bench_dwbc.py --test squat_pick_ground --trials 15 \
        --out-dir results/dwbc/squat_pick_ground --video policy
    MUJOCO_GL=egl python bench_dwbc.py --test vln_follow --trials 10 \
        --tapes vln_tapes.json --out-dir results/dwbc/vln_follow --video policy

License note: GR00T-WholeBodyControl weights ship under the NVIDIA Open Model
License. Internal benchmark use only.
"""
import argparse
import collections
import json
...
# ---------------------------------------------------------------------------
# Constants — GR00T decoupled-WBC pipeline (sources: g1_gear_wbc.yaml /
# run_mujoco_gear_wbc.py; transplanted verbatim from the validated
# smoke_dwbc.py — DO NOT change the obs math).
# ---------------------------------------------------------------------------
FRAMEWORK = "gr00t-wbc"
# --label overrides the framework name written into JSONL / summary / video
# filenames (mirrors bench_falcon's LABEL). Set once in main() before any
# trial runs, read-only afterwards.
LABEL = "gr00t-wbc"
# scene_29dof.xml on the 4090 (same scene bench_agile / bench_falcon use);
# overridable via --mjcf. The two GR00T onnx weights live in --model-dir.
...
DEFAULT_MODEL_DIR = os.environ.get(
    "DWBC_MODEL_DIR",
    "/sda/lizhe/g1bench/Psi0/third_party/SIMPLE/third_party/"
    "decoupled_wbc/sim2mujoco/resources/robots/g1/policy")
BALANCE_ONNX = "GR00T-WholeBodyControl-Balance.onnx"
WALK_ONNX = "GR00T-WholeBodyControl-Walk.onnx"
# Balance/Walk switch: ||loco_cmd[vx,vy,wz]|| <= this -> Balance policy.
LOCO_CMD_DEADZONE = 0.05

SIM_DT = 0.005            # g1_gear_wbc.yaml: simulation_dt
DECIMATION = 4            # control_decimation -> 50 Hz policy
POLICY_DT = SIM_DT * DECIMATION

N_JOINTS = 29             # legs 12 + waist 3 + 2x7 arms (scene_29dof actual)
N_ACTIONS = 15            # policy controls the lower body: legs 12 + waist 3
N_ARMS = 14               # arms PD-held at arm_target
# obs layout (smoke_dwbc.single_obs): loco_cmd*scale(3) + height(1) + rpy(3) +
# ang_vel(3) + grav(3) + dof_pos(29) + dof_vel(29) + last_action(15) = 86
SINGLE_OBS_DIM = 3 + 1 + 3 + 3 + 3 + N_JOINTS + N_JOINTS + N_ACTIONS  # = 86
HIST_LEN = 6
OBS_DIM = SINGLE_OBS_DIM * HIST_LEN                                   # = 516

# Lower-body (legs L/R hip_pitch,hip_roll,hip_yaw,knee,ankle_pitch,ankle_roll
# then waist_yaw,waist_roll,waist_pitch) default + PD gains; the 15-action
# policy output * ACTION_SCALE + LOWER_DEFAULT = the per-step lower targets.
LOWER_DEFAULT = np.array([-0.1, 0.0, 0.0, 0.3, -0.2, 0.0,
                          -0.1, 0.0, 0.0, 0.3, -0.2, 0.0,
                          0.0, 0.0, 0.0], dtype=np.float64)
LOWER_KP = np.array([150, 150, 150, 200, 40, 40,
                     150, 150, 150, 200, 40, 40,
                     250, 250, 250], dtype=np.float64)
LOWER_KD = np.array([2, 2, 2, 4, 2, 2,
                     2, 2, 2, 4, 2, 2,
                     5, 5, 5], dtype=np.float64)

# Arms (14) PD-held at arm_target (zeros, or psi0-replayed angles). Scalar
# gains broadcast over all 14 arm joints (smoke_dwbc: kp=100, kd=0.5).
ARM_KP = 100.0
ARM_KD = 0.5

# Obs scales (smoke_dwbc / g1_gear_wbc.yaml). ang_vel scale is a CLI flag for
# parity with the other harnesses; the GR00T default is 0.5 (NOT homie's 0.25).
CMD_SCALE = np.array([2.0, 2.0, 0.5], dtype=np.float32)
DOF_POS_SCALE = 1.0
DOF_VEL_SCALE = 0.05
ACTION_SCALE = 0.25
OBS_CLIP = 100.0          # defensive obs clip (no fall-poison)
ACTION_CLIP = 100.0       # defensive action clip

STAND_HEIGHT = 0.74       # height command for standing/walking
SQUAT_HEIGHT = 0.45       # T2 low target (GR00T squat domain)
INIT_BASE_Z = 0.78        # init pelvis height (feet clear of the floor at the
                          # default leg angles; verified via smoke_dwbc)
...
# Low forward-reach arm pose (FK-tuned; see NOTES): wrists land at ~(0.30,
# +/-0.18, base_z + 0.07) — y straddles the 0.35 m box width. dwbc PD-holds
# the 14 arms only (waist is policy-controlled), so this is 14-dim.
# Order: L[sh_p, sh_r, sh_y, elbow, wr x3], R[mirror].
PICK_ARM_POSE = np.array([-0.7, 0.20, 0.0, 1.2, 0.0, 0.0, 0.0,
...
# Wrist link names that ms.WRIST_L/WRIST_R placeholders sed-replace to.
...
# ---- psi0 arm replay (contract v1; psi0_replay/make_replay.py) -------------
# npz fields: t (N,), upper_names (K,), upper_pos (N,K), height_cmd (N,),
# grasp_close_t (scalar), carry_pose (K,), meta_json; dt = 0.02 s. agile29:
# K=17 = arms14 + [waist_yaw, waist_roll, waist_pitch]. dwbc PD-holds ONLY the
# 14 arms (the waist is policy-controlled), so prepare_upper_replay() maps the
# 14 arm columns by joint NAME onto the qpos[22:36] arm slots and DROPS the 3
# waist columns. dt = 0.02 s == POLICY_DT (0.005 * 4) — no resampling.
PSI0_GRASP_DIST_M = 0.30  # both wrists must be closer than this to the box surface
PSI0_HEIGHT_CLIP = (0.24, STAND_HEIGHT)  # GR00T height domain for replayed height_cmd
...
# Collision masks (unified psi0 spec): robot keeps bit 1 (scene_29dof collision
...
# front of the torso). 14-dim (dwbc PD-holds only the arms; the waist is
# policy-controlled). Order: L[sh_pitch, sh_roll, sh_yaw, elbow, wr x3],
# R[mirror]. NOTE: the obs subtracts the TRAINING defaults (zeros for the
# arms) — never these PD targets.
BOX_ARM_POSE = np.array([-0.4, 0.12, 0.0, 1.2, 0.0, 0.0, 0.0,
                         -0.4, -0.12, 0.0, 1.2, 0.0, 0.0, 0.0],
                        dtype=np.float64)
ZERO_ARM_POSE = np.zeros(N_ARMS, dtype=np.float64)

# Obs default subtraction (smoke_dwbc padded_default): lower at LOWER_DEFAULT
# (15), arms at 0 (14) -> 29-dim, subtracted from qpos[7:36] in the obs.
OBS_DEFAULT_29 = np.concatenate(
    [LOWER_DEFAULT, np.zeros(N_ARMS)]).astype(np.float32)
...
# ---------------------------------------------------------------------------
# Policy loading (dual ONNX: Balance + Walk; CPU, numpy I/O)
# ---------------------------------------------------------------------------
def load_policies(model_dir):
    """Load the GR00T Balance + Walk onnx sessions (CPU). Returns a dict with
    both sessions + their input/output names. select_policy() picks per tick."""
    so = ort.SessionOptions()
    so.intra_op_num_threads = 1     # determinism + tiny MLP
    out = {}
    for key, fname in (("balance", BALANCE_ONNX), ("walk", WALK_ONNX)):
        path = os.path.join(model_dir, fname)
        if not os.path.isfile(path):
            sys.stderr.write("ERROR: GR00T weight not found: %s\n" % path)
            sys.exit(2)
        sess = ort.InferenceSession(path, sess_options=so,
                                    providers=["CPUExecutionProvider"])
        out[key] = sess
        out[key + "_in"] = sess.get_inputs()[0].name
        out[key + "_out"] = sess.get_outputs()[0].name
    return out


def policy_step(pol, loco_cmd, obs516):
    """Run Balance (||loco_cmd|| <= deadzone) or Walk on the 516-d obs, return
    the 15-d action (numpy). Mirrors smoke_dwbc's per-tick policy select."""
    if float(np.linalg.norm(loco_cmd)) <= LOCO_CMD_DEADZONE:
        sess, inn, outn = pol["balance"], pol["balance_in"], pol["balance_out"]
    else:
        sess, inn, outn = pol["walk"], pol["walk_in"], pol["walk_out"]
    return sess.run([outn], {inn: obs516[None, :].astype(np.float32)})[0][0]


# ---------------------------------------------------------------------------
# Scene building (scene_29dof.xml + free-body / furniture XML injection)
# ---------------------------------------------------------------------------
def find_mjcf(arg_mjcf):
    candidates = [
        arg_mjcf,
        os.environ.get("UNITREE_MUJOCO_SCENE"),
        DEFAULT_MJCF,
    ...
    sys.stderr.write(
        "ERROR: scene_29dof.xml not found. Pass --mjcf /path/to/scene_29dof.xml"
        " (must be the SCENE file with the floor + robot include)\n")
    sys.exit(2)
...
def _compile_sibling(mjcf_path, xml, suffix):
    """Write the injected XML to a sibling of the scene file (so the relative
    <include>/meshdir paths still resolve) and compile from_xml_path."""
    out_path = os.path.join(
        os.path.dirname(mjcf_path),
        os.path.splitext(os.path.basename(mjcf_path))[0] + suffix)
    with open(out_path, "w") as f:
        f.write(xml)
    return mujoco.MjModel.from_xml_path(out_path)


def build_model(mjcf_path, test):
    with open(mjcf_path, "r") as f:
        xml = f.read()
    if test in BOX_TESTS:
        # free box injected at the scene-worldbody FLOOR_ANCHOR — AFTER the
        # robot <include>, so the robot keeps qpos[0:36]/qvel[0:35] and the box
        # freejoint lands at the tail (qpos[36:]). psi0 walk/circle/pick tests
        # use the cracker-box variant (same names, different size/mass/bits);
        # squat_place_psi0 keeps the v2 2 kg carry box (spec v4);
        # squat_pick_ground uses the v2-size box with floor collision (spec v5).
    ...
    model = _compile_sibling(mjcf_path, xml, "__bench_%s.xml" % test)
...
def configure_box_welds(model, upper_pose=BOX_ARM_POSE):
    ...
    `upper_pose` is the 14-dim qpos[22:36] ARM pose the welds are anchored at:
    ...
    assert qadr == 36 and vadr == 35, \
        "box freejoint must come after the 36/35 robot coords (got %d/%d)" \
        % (qadr, vadr)
    ...
    # nominal noise-free standing pose, lower at LOWER_DEFAULT, arms at anchor
    d = mujoco.MjData(model)
    d.qpos[2] = INIT_BASE_Z
    d.qpos[3] = 1.0
    d.qpos[7:22] = LOWER_DEFAULT
    d.qpos[22:36] = upper_pose
    d.qpos[qadr + 3] = 1.0
...
def load_upper_replay(path):
    ...
    arm mapping (pos_arm/carry_arm) is filled by prepare_upper_replay()."""
    if not os.path.isfile(path):
...
def prepare_upper_replay(model, rep):
    ...
    slots (dwbc PD-holds only the arms; the waist stays policy-controlled, so
    the npz's waist columns are DROPPED). Returns a NEW dict with pos_arm
    (N,14) / carry_arm (14,) added; arm slots with no replay column stay at the
    training default 0. The agile29 npz K=17 (arms14 + waist3) -> 14 mapped, 3
    waist columns ignored."""
    idx = []          # arm-slot index 0..13 for each MAPPED replay column
    cols = []         # corresponding replay column index
    for col, name in enumerate(rep["names"]):
        ...
        if 0 <= k < N_ARMS:
            idx.append(k)
            cols.append(col)
        # else: waist (qpos[19:22], k<0) or out-of-range -> dropped silently
...
def run_trial(ctx, trial_idx, seed, vx_target=1.0, direction="ccw",
    ...
    m = ctx["model"]
    ids = ctx["ids"]
    pol = ctx["pol"]              # dual-policy dict (balance + walk)
    ang_vel_scale = ctx["ang_vel_scale"]
    rep = ctx.get("upper")        # psi0 arm replay dict, else None
    ...
    if test in PSI0_TESTS:
        arm_target = rep["pos_arm"][0]   # replay frame 0 (PD targets follow)
    elif test in CARRY_TESTS:
        arm_target = BOX_ARM_POSE
    else:
        arm_target = ZERO_ARM_POSE
    ...
    # lower body (legs 12 + waist 3) at LOWER_DEFAULT + init noise
    d.qpos[7:22] = LOWER_DEFAULT + rng.uniform(-JOINT_NOISE, JOINT_NOISE, 15)
    if test in PSI0_TESTS:
        # replayed arms start exactly at replay frame 0 (no noise) so the
        # per-frame PD targets do not snap at t=0.
        d.qpos[22:36] = arm_target
    else:
        d.qpos[22:36] = arm_target + rng.uniform(-JOINT_NOISE, JOINT_NOISE,
                                                 N_ARMS)
    if box is not None:
    ...
    # --- policy state (mirrors smoke_dwbc) --------------------------------
    action = np.zeros(N_ACTIONS, dtype=np.float32)   # last raw policy output
    target_lower = LOWER_DEFAULT.copy()              # PD targets for qpos[7:22]
    ...
    try:
        for step in range(max_steps):
            # PD every sim step (200 Hz), identical split to smoke_dwbc:
            # lower body (legs+waist) from the policy targets, arms held at
            # arm_target. scene_29dof actuators are torque/motor.
            d.ctrl[0:15] = ((target_lower - d.qpos[7:22]) * LOWER_KP
                            - d.qvel[6:21] * LOWER_KD)
            d.ctrl[15:29] = ((arm_target - d.qpos[22:36]) * ARM_KP
                             - d.qvel[21:35] * ARM_KD)
            mujoco.mj_step(m, d)
            t = (step + 1) * SIM_DT
            t_end = t
            ...
            # psi0: arm PD targets replayed frame-by-frame at 50 Hz (the
            # 200 Hz PD above holds the latest target between ticks). The
            # waist stays policy-controlled (not replayed).
            if rep is not None:
                if test in ("squat_box_psi0", "squat_place_psi0"):
                    ...
                    if k < 0:
                        arm_target = rep["pos_arm"][0]
                    elif k >= rep["n"]:
                        arm_target = (rep["carry_arm"]
                                      if test == "squat_box_psi0"
                                      else rep["pos_arm"][-1])
                    else:
                        arm_target = rep["pos_arm"][k]
                else:
            ...
            # (the obs subtracts the TRAINING defaults — zeros — not these
            # targets, same rule as the carry tests).
            if test == "squat_pick_ground":
                f_arm = min(max((t - SETTLE_S) / PICK_REACH_S, 0.0), 1.0)
                arm_target = ((1.0 - f_arm) * ZERO_ARM_POSE
                              + f_arm * PICK_ARM_POSE)
            ...
            # obs frame — exact mirror of smoke_dwbc.single_obs (86-d):
            #   [0:3] loco_cmd*CMD_SCALE  [3] height_cmd  [4:7] rpy_cmd(0,0,0)
            #   [7:10] qvel[3:6]*ang_vel  [10:13] projected_gravity
            #   [13:42] (qpos[7:36]-padded_default)*DOF_POS_SCALE  (29)
            #   [42:71] qvel[6:35]*DOF_VEL_SCALE  (29)
            #   [71:86] last raw action  (15)
            ...
            loco_cmd = np.array([vx_c, vy_c, wz_c], dtype=np.float32)
            single = np.zeros(SINGLE_OBS_DIM, dtype=np.float32)
            single[0:3] = loco_cmd * CMD_SCALE
            single[3] = h_c                        # height command, scale 1.0
            single[4:7] = 0.0                      # rpy_cmd (0, 0, 0)
            single[7:10] = omega * ang_vel_scale
            single[10:13] = grav
            single[13:13 + N_JOINTS] = (qj - OBS_DEFAULT_29) * DOF_POS_SCALE
            single[13 + N_JOINTS:13 + 2 * N_JOINTS] = dqj * DOF_VEL_SCALE
            single[13 + 2 * N_JOINTS:SINGLE_OBS_DIM] = action  # prev raw output
            np.clip(single, -OBS_CLIP, OBS_CLIP, out=single)
            obs_hist.append(single)
            obs516 = np.concatenate(obs_hist).astype(np.float32)  # oldest->newest

            out = policy_step(pol, loco_cmd, obs516)
            action = np.clip(out.astype(np.float32), -ACTION_CLIP, ACTION_CLIP)
            target_lower = action.astype(np.float64) * ACTION_SCALE + LOWER_DEFAULT
        ...
        if os.environ.get("DWBC_DEBUG_TB"):
            import traceback
            traceback.print_exc()
    finally:
...
# ===========================================================================
# ManipArena v2 (BENCHMARK_V2_DESIGN §2-§4). GR00T decoupled-WBC adapter:
#   build_arena_model -> inject ms.furniture_xml into scene_29dof.xml
#   configure_arena_welds -> resolve free-body addresses + weld eq ids
#   DwbcMissionIO(MissionIO) -> the ~13 callbacks manip_mission.Mission drives
#   run_arena_trial -> spawn at the spec pose, drive the mission, score it
# ===========================================================================
def reach_base_h(top_h):
    """GR00T height command to bring the wrists to a surface at height
    ``top_h`` (table top / store-pad floor). Linear "lower surface -> lower
    base" with a fixed reach offset, clipped to the trackable domain
    [ARENA_H_MIN, ARENA_STAND_H]. The mission emits this as its squat target;
    the depth floor of the policy decides whether the low tables are reachable
    at all (the whole point of the H_pick sweep)."""
    return float(min(max(top_h + ARENA_REACH_OFFSET, ARENA_H_MIN),
                     ARENA_STAND_H))


def build_arena_model(mjcf_path, spec):
    """Compile scene_29dof.xml with the ManipArena furniture + free bodies.

    Mechanics (mirrors build_model's bit-2 path, BENCHMARK_V2_DESIGN §3):
      - furniture bodies appended AFTER the robot <include> (at FLOOR_ANCHOR) so
        the box/cube freejoints land at the qpos/qvel TAIL
        (FREE_BODY_QPOS_LAYOUT); the robot keeps qpos[0:36]/qvel[0:35] and the
        29-joint obs slice (qpos[7:36]/qvel[6:35]) is never polluted.
      - the <equality> weld block inserted before </mujoco>.
      - ms.WRIST_L/WRIST_R placeholders replaced with the wrist link names so
        the wrist<->box welds compile against real bodies.
    ...
    model = _compile_sibling(mjcf_path, xml,
                             "__arena_v%d.xml" % spec.variant_seed)
    model.opt.timestep = SIM_DT
...
def configure_arena_welds(model):
    ...
    # robot end = 36/35 for the 29dof scene; box then cube at the tail.
    assert box_qadr == 36 and cube_qadr == 43, \
...
class DwbcMissionIO:
    """MissionIO (manip_mission.py Protocol) bound to a live GR00T MjModel/
    ...
    Heights are GR00T height commands; wz is the native yaw-rate channel. The
    14 arms are driven by a real_ep053 Psi0 arm replay (set via replay_upper);
    the rollout reads io.arm_target each tick and runs the arm PD (the waist
    stays policy-controlled in the 15-action lower output)."""
    ...
    def __init__(self, m, d, box, ids, replay, h_stand=ARENA_STAND_H):
        ...
        self.replay = replay            # prepared real_ep053 dict (pos_arm etc.)
        self.h_stand = h_stand
        # mutable slots the rollout reads:
        self.cmd = (0.0, 0.0, 0.0, h_stand)        # vx, vy, wz, height
        self.arm_target = (replay["pos_arm"][0] if replay is not None
...
# Mission/DwbcMissionIO instead of command_at.
# ---------------------------------------------------------------------------
def run_arena_trial(ctx, spec, trial_idx, seed, video_path=None):
    ...
    # --- mission + io -----------------------------------------------------
    io = DwbcMissionIO(m, d, box, ids, rep, h_stand=ARENA_STAND_H)
    io.arm_target = arm_target
    ...
    try:
        for step in range(max_steps):
            d.ctrl[0:15] = ((target_lower - d.qpos[7:22]) * LOWER_KP
                            - d.qvel[6:21] * LOWER_KD)
            d.ctrl[15:29] = ((io.arm_target - d.qpos[22:36]) * ARM_KP
                             - d.qvel[21:35] * ARM_KD)
            mujoco.mj_step(m, d)
            t = (step + 1) * SIM_DT
            t_end = t
            ...
            # obs frame — identical to the standard rollout (qpos[7:36] slice is
            ...
            single[13:13 + N_JOINTS] = (qj - OBS_DEFAULT_29) * DOF_POS_SCALE
            single[13 + N_JOINTS:13 + 2 * N_JOINTS] = dqj * DOF_VEL_SCALE
            single[13 + 2 * N_JOINTS:SINGLE_OBS_DIM] = action
            np.clip(single, -OBS_CLIP, OBS_CLIP, out=single)
            obs_hist.append(single)
            obs516 = np.concatenate(obs_hist).astype(np.float32)
...
def run_arena_main(args, mjcf, model_dir, out_dir, video_dir):
    ...
    14 arms are a real_ep053 Psi0 arm replay (required, --upper-replay)."""
    if args.upper_replay is None:
        sys.stderr.write("ERROR: --upper-replay is required for %s "
                         "(real_ep053 agile29 npz, contract v1)\n" % args.test)
        sys.exit(2)
    ...
    if replay["embodiment"] not in ("agile29", "unknown"):
        raise SystemExit("upper-replay embodiment %r != agile29 — wrong npz?"
                         % replay["embodiment"])
    ...
    model = build_arena_model(mjcf, spec)
    # robot 36/29 + box freejoint (7) + cube freejoint (7) = 50 qpos, 29 actuators
    assert model.nq == 50 and model.nu == 29, \
...
def main():
    ap = argparse.ArgumentParser(
        description="GR00T decoupled-WBC G1 benchmark harness")
    ...
    ap.add_argument("--upper-replay", default=None,
                    help="psi0 tests: upper_replay_agile29_*.npz (contract "
                         "v1; 4090:/sda/lizhe/g1bench/psi0_replay/out/). dwbc "
                         "replays the 14 arm columns; the 3 waist columns are "
                         "dropped (waist is policy-controlled).")
    ...
    ap.add_argument("--video", choices=["none", "policy", "all"],
                    ...
                    help="policy = first %d successful + all failed trials"
                         % MAX_OK_VIDEOS)
    ap.add_argument("--label", default="gr00t-wbc",
                    help="framework name written into JSONL / summary / video "
                         "filenames (default gr00t-wbc)")
    ap.add_argument("--mjcf", default=None,
                    help="scene_29dof.xml (the SCENE file with floor + robot "
                         "include; default %s)" % DEFAULT_MJCF)
    ap.add_argument("--model-dir", default=DEFAULT_MODEL_DIR,
                    help="dir with GR00T-WholeBodyControl-{Balance,Walk}.onnx "
                         "(default %s)" % DEFAULT_MODEL_DIR)
    ap.add_argument("--ang-vel-scale", type=float, default=0.5,
                    help="obs ang-vel scale. 0.5 = GR00T g1_gear_wbc.yaml "
                         "(default; matches smoke_dwbc).")
    ...
    global CIRCLE_VX, CIRCLE_WZ, CUSTOM_VX_REL_THR, LABEL
    LABEL = args.label
    ...
    mjcf = find_mjcf(args.mjcf)
    model_dir = args.model_dir
    out_dir = args.out_dir or os.path.join("dwbc_results", args.test)
    ...
    if args.test in ARENA_TESTS:
        run_arena_main(args, mjcf, model_dir, out_dir, video_dir)
        return
    ...
    if args.test in PSI0_TESTS:
        ...
        replay = load_upper_replay(args.upper_replay)
        if replay["embodiment"] not in ("agile29", "unknown"):
            raise SystemExit("upper-replay embodiment %r != agile29 — "
    ...
    model = build_model(mjcf, args.test)
    # robot is 36/29; box tests add a free box body (+7 qpos, 0 actuators)
    expect_nq = 43 if args.test in BOX_TESTS else 36
    assert model.nq == expect_nq and model.nu == 29, \
    ...
    pol = load_policies(model_dir)   # dual onnx: balance + walk [batch,516]->15
    ctx = {
        "test": args.test,
        "model": model,
        "ids": lookup_ids(model),
