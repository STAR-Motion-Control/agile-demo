...
> ⚠️ **顺序:先 SAM3 再底座**(已知问题,待排查)。先起底座、再起 SAM3,会在 SAM3 加载模型时让机器人乱跳(疑似 SAM3 抢 GPU 卡顿 stall 了 adapter 控制环)。三个服务各占一终端、**前台跑(去掉 `&`)**,好看日志、别忘关。

```bash
# 1) ★SAM3 服务(必须最先起,前台跑;另开终端验证: curl -s 127.0.0.1:5300/docs >/dev/null && echo SAM3_OK)
cd ~/sam3 && ~/sam3/.venv/bin/uvicorn scripts.sam_server:app --host 0.0.0.0 --port 5300

# 2) GR00T 底座(merger + adapter;等 SAM3 加载完再起。起来后 Ctrl-C 掉 pane3 键盘,只留 merger + adapter)
...
# 3) HTTP IPC 桥(收机器人 HTTP 命令 → 写 units:agile IPC 给 adapter;前台跑,自检 curl 127.0.0.1:5001/status)
