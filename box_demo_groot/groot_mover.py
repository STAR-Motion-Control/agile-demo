...
class GrootMover:
    ...
    def _write_active_command(
        ...
        **kwargs,
        ...
        # stop() sets the event before taking this lock. Therefore either this
        # command completes before stop's zero frame, or it is suppressed.
        with self._command_lock:
    ...
    def _settle(self, allow_recovery: bool | None = None, cancel_event=None) -> bool:
        ...
        while time.monotonic() < end:
            ...
            if self._cancel_requested(cancel_event):
                return False
        if allow_recovery:
            return self._wait_for_recovery(cancel_event=cancel_event)
        return True
    ...
    def _wait_for_recovery(self, cancel_event=None) -> bool:
        ...
        while time.monotonic() < deadline:
            ...
            if not active:
                return True
            if self._motion_cancel.wait(0.05):
                return False
    ...
    def _guard_walk_height(self, cancel_event=None) -> bool:
        ...
        if self.auto_raise_for_walk:
            ...
            # Serialize the height write with stop(). stop sets cancellation
            # before taking this lock, so a late height command cannot follow
            # stop's zero frame.
            with self._command_lock:
                if self._cancel_requested(cancel_event):
                    return False
    ...
    def stop(self) -> None:
        ...
        self._write_command("RL_FULL", height=self._height, allow_recovery=False)
    ...
    def publish_velocity(
        ...
        *,
        ...
        if not self._write_external_command(
            "RL_FULL",
            forward,
            lateral,
            yaw,
            ...
            raise MotionCancelled("continuous command cancelled")
