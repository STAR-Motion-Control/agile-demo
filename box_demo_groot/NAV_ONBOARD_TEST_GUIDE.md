...
只有当前 `/planned_action` 批次全部执行完成、且没有新规划抢占时，executor 才调用一次
`finish_segment` 允许检查。这样四个 action 中间不会插入反向踏步。键盘 `o`、HTTP `/damp`、人工
`/stop` 和导航取消仍是不回正的安全停止。普通版和 `_taptap` 版的 mover 都使用 `0.4 s` 停止保持，
